...
function handleRequest(e) {
  try {
    ...
    if (p.action === 'update_finnegans_asiento')  return updateFinnegansAsiento(p);
...
function writeCosecha(p) {
  ...
  const sobreescribir = p.sobreescribir === '1';
  ...
  // Para camiones: verificar duplicados (salvo que sea un regrabado explícito)
  const sobreescribir = p.sobreescribir === '1';
  if (!sobreescribir && p.destino === 'cam' && cpSilo) {
...
// ══════════════════════════════════════
// FINNEGANS — Guardar asiento por CP
// ══════════════════════════════════════

function updateFinnegansAsiento(p) {
  // Busca la fila en la hoja de Traslados que coincide con la CP
  // y guarda el número de asiento de Finnegans
  const ss = getUserSS(p);
  const camp = (p.campania || '').replace('/', '_');
  const sheetName = 'T_' + p.campo + '-' + p.cultivo + '-' + camp;
  const sh = ss.getSheetByName(sheetName);
  ...
  const cp           = (p.cp || '').trim();
  const finnAsiento  = (p.finnAsiento || '').trim();
  if (!cp)          return { ok: false, error: 'Falta cp' };
  if (!finnAsiento) return { ok: false, error: 'Falta finnAsiento' };

  const data = sh.getDataRange().getValues();
  const headers = data[0].map(h => String(h).trim());

  // Asegurar que existe la columna "Asiento Finnegans"
  let idxFinn = headers.indexOf('Asiento Finnegans');
  if (idxFinn === -1) {
    // Agregar la columna al final
    idxFinn = headers.length;
    sh.getRange(1, idxFinn + 1).setValue('Asiento Finnegans');
    sh.getRange(1, idxFinn + 1).setBackground('#1565C0').setFontColor('#FFFFFF').setFontWeight('bold');
  }

  const idxCP = headers.indexOf('Carta de Porte');
  if (idxCP === -1) return { ok: false, error: 'No se encontró columna Carta de Porte' };

  let filaActualizada = -1;
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idxCP]).trim() === cp) {
      sh.getRange(i + 1, idxFinn + 1).setValue(finnAsiento);
      sh.getRange(i + 1, idxFinn + 1)
  ...
  if (filaActualizada === -1) {
    return { ok: false, error: 'CP no encontrada en la hoja: ' + cp };
  }

  return { ok: true, cp, finnAsiento, fila: filaActualizada };
}
